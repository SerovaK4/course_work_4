...
class Connector:
    ...
    def select(self):
        with open(self.__filename, 'r', encoding="utf-8") as file:
            data = json.load(file)
        vacancies = [Vacancy(x['vacancy_id'], x['title'], x['salary_from'], x['salary_to'], x['employer'], x['api']) for x in data]
        return vacancies
...
class HeadHunter(Engine):
    def __init__(self, keyword):
        ...
        self.__header = {
            "User-Agent": "User-Agent: MyApp/1.0 (my-app-feedback@example.com)"
        }
        self.__params = {
            "text": keyword,
            "page": 0,
    ...
    def get_salary(salary):
        ...
        if salary and salary['to'] and salary['to'] != 0:
            formatted_salary[0] = salary['to'] if salary['currency'].lower() == 'rur' else salary['to'] * 76
    ...
    def get_vacancies(self, pages_count=1):
        while self.__params['pages'] < pages_count:
            ...
            self.__vacancies.expend(values)
            self.__params['pages'] += 1
